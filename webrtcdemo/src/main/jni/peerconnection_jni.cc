    ...
    private:
        ...
        static void PopulateConstraintsFromJavaPairList(
            ...
            jfieldID j_id = GetFieldID(jni,
                                       GetObjectClass(jni, j_constraints), field_name,
                                       "Ljava/util/List;");
...
JOW(jstring, DataChannel_label
)(
JNIEnv *jni, jobject
j_dc) {
return
JavaStringFromStdString(jni, ExtractNativeDC(jni, j_dc)
->
...
JOW(jlong, DataChannel_bufferedAmount
)(
JNIEnv *jni, jobject
j_dc) {
...
bool ret = ExtractNativeDC(jni, j_dc)->Send(DataBuffer(
        ...
        binary));
jni->
ReleaseByteArrayElements(data, bytes, JNI_ABORT
);
return
ret;
}
...
JOW(void, PeerConnection_freePeerConnection)(JNIEnv * , jclass, jlong

j_p) {
CHECK_RELEASE(reinterpret_cast<PeerConnectionInterface*>(j_p));
}

JOW(void, PeerConnection_freeObserver)(JNIEnv * , jclass, jlong
...
JOW(void, MediaSource_free)(JNIEnv * , jclass, jlong

j_p) {
CHECK_RELEASE(reinterpret_cast<MediaSourceInterface*>(j_p));
}

JOW(void, VideoRenderer_freeWrappedVideoRenderer)(JNIEnv * , jclass, jlong
...
JOW(void, MediaStreamTrack_free)(JNIEnv * , jclass, jlong
...
JOW(jstring, MediaStream_nativeLabel
)(
JNIEnv *jni, jclass, jlong
j_p) {
return
JavaStringFromStdString(
        jni,
reinterpret_cast<MediaStreamInterface*>(j_p)->
...
JOW(void, MediaStream_free)(JNIEnv * , jclass, jlong
...
JNIEnv *jni, jclass, jobject
j_observer) {
return (jlong)new
PCOJava(jni, j_observer
);
}
...
JNIEnv *jni, jclass, jobject
context,
jboolean initialize_audio, jboolean
initialize_video,
...
if (initialize_video) {
failure |=
AndroidVideoCapturerJni::SetAndroidObjects(jni, context
);
}
if (initialize_audio)
failure |=

webrtc::VoiceEngine::SetAndroidObjects(GetJVM(), context
...
LOG(LS_INFO)
<< "Starting internal tracing to: " <<
init_string;
bool ret = rtc::tracing::StartInternalCapture(init_string);
jni->
ReleaseStringUTFChars(j_event_tracing_filename, init_string
);
return
ret;
}
...
RTC_CHECK(factory)
<< "Failed to create the peer connection factory; "
...
return
jlongFromPointer(owned_factory);
}

JOW(void, PeerConnectionFactory_nativeFreeFactory)(JNIEnv * , jclass, jlong
...
static PeerConnectionFactoryInterface *factoryFromJava(jlong
j_p) {
...
if (!certificate) {
LOG(LS_ERROR)
<< "Failed to generate certificate. KeyType: " <<
key_type;
return 0;
}
...
observer->SetConstraints(new
ConstraintsWrapper(jni, j_constraints
));
...
JOW(jobject, PeerConnection_getLocalDescription
)(
JNIEnv *jni, jobject
j_pc) {
...
return sdp ?
JavaSdpFromNativeSdp(jni, sdp
) :
NULL;
}

JOW(jobject, PeerConnection_getRemoteDescription
)(
JNIEnv *jni, jobject
j_pc) {
const SessionDescriptionInterface *sdp =
...
CHECK_EXCEPTION(jni)
<< "error during NewObject";
// Channel is now owned by Java object, and will be freed from there.
int bumped_count = channel->AddRef();
RTC_CHECK(bumped_count
== 2) << "Unexpected refcount";
return
j_channel;
}
...
static SessionDescriptionInterface *JavaSdpToNativeSdp(
    ...
    jstring
    j_description = (jstring)
    GetObjectField(jni, j_sdp, j_description_id);
...
return
ExtractNativePC(jni, j_pc
)->
SetConfiguration(rtc_config);
}
...
candidates.
push_back(GetCandidateFromJava(jni, j_candidate));
}
return
ExtractNativePC(jni, j_pc
)->
RemoveIceCandidates(candidates);
}
...
JOW(jobject, PeerConnection_nativeGetSenders
)(
JNIEnv *jni, jobject
j_pc) {
...
jni->
CallBooleanMethod(j_senders, j_array_list_add, j_sender
);
CHECK_EXCEPTION(jni)
...
JOW(jobject, PeerConnection_nativeGetReceivers
)(
JNIEnv *jni, jobject
j_pc) {
jclass j_array_list_class = FindClass(jni, "java/util/ArrayList");
...
jni->
CallBooleanMethod(j_receivers, j_array_list_add, j_receiver
);
CHECK_EXCEPTION(jni)
<< "error during CallBooleanMethod";
}
...
JOW(jobject, PeerConnection_signalingState
)(
JNIEnv *jni, jobject
j_pc) {
...
return
JavaEnumFromIndex(jni,
"PeerConnection$SignalingState", state);
}

JOW(jobject, PeerConnection_iceConnectionState
)(
JNIEnv *jni, jobject
j_pc) {
...
JOW(jobject, PeerConnection_iceGatheringState
)(
JNIEnv *jni, jobject
j_pc) {
...
JOW(jobject, MediaSource_nativeState
)(
JNIEnv *jni, jclass, jlong
j_p) {
...
JOW(jstring, MediaStreamTrack_nativeId
)(
JNIEnv *jni, jclass, jlong
j_p) {
return
...
JOW(jstring, MediaStreamTrack_nativeKind
)(
JNIEnv *jni, jclass, jlong
j_p) {
return
...
JOW(jboolean, MediaStreamTrack_nativeEnabled
)(
JNIEnv *jni, jclass, jlong
j_p) {
...
JOW(jobject, MediaStreamTrack_nativeState
)(
JNIEnv *jni, jclass, jlong
j_p) {
return
...
rtc::LogMessage::AddLogToStream(
        sink,
static_cast
<rtc::LoggingSeverity>(j_severity)
);
return (jlong)
sink;
}
...
LOG_V(rtc::LoggingSeverity::LS_WARNING)
<<
"Failed to open CallSessionFileRotatingStream for path " <<
dir_path;
...
LOG_V(rtc::LoggingSeverity::LS_WARNING)
<<
"CallSessionFileRotatingStream returns 0 size for path " <<
dir_path;
return jni->NewByteArray(0);
}
...
webrtc::RtpParameters parameters =
        reinterpret_cast<RtpSenderInterface *>(j_rtp_sender_pointer)
                ->GetParameters();
return
JsepRtpParametersToJavaRtpParameters(jni, parameters
);
}
